...
import { s as script$3 } from './index-Bkc7jbaS.mjs';
import { s as script$2 } from './index-CI0zemKa.mjs';
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  if (!$data.detailedView && !$data.fetching) {
    ...
    if ($options.filteredQuotation.length > 0 && $options.userData.shopOwnerVerified) {
      _push(`<div class="max-h-[600px] overflow-y-auto"><!--[-->`);
      ssrRenderList($options.filteredQuotation, (order, index) => {
        _push(`<div class="border rounded-lg mb-4 p-4 cursor-pointer"><div class="flex justify-between items-center"><span class="${ssrRenderClass([{
          ...
          "bg-green-100 text-green-600": order.status === "Delivered",
          "bg-gray-300 text-gray-700": order.status === "Cancelled"
        }, "px-3 py-1 text-xs rounded-full"])}">${ssrInterpolate(order.status)}</span><span class="text-sm text-gray-500">${ssrInterpolate($options.formatDate(order.createdAt))}</span></div><div class="mt-4"><p class="text-red-600 font-semibold"> Order ID: ${ssrInterpolate(order.id.slice(0, 8))}</p><!--[-->`);
        ssrRenderList(order.orderItems, (item2, itemIndex) => {
          var _a;
          _push(`<div class="mt-2 flex items-center"><img${ssrRenderAttr("src", (_a = item2.productModel.images[0]) == null ? void 0 : _a.uploadUrl)} alt="Product Image" class="w-16 h-16 rounded-md object-cover"><div class="ml-4"><p class="text-gray-700 font-medium">${ssrInterpolate(item2.productModel.name)}</p><p class="text-sm text-gray-500">Qty: ${ssrInterpolate(item2.quantity)}</p></div></div>`);
        });
        _push(`<!--]--></div></div>`);
      });
      _push(`<!--]--></div>`);
    ...
    if ($data.fetching) {
      ...
      ssrRenderList($data.orderDetails.orderItems, (item2, index) => {
        ...
        )} alt="Product Image" class="w-16 h-16 rounded-md object-cover"><div><p class="text-lg font-semibold">${ssrInterpolate(item2.productModel.name)}</p><p class="text-gray-500"><span>${(_a2 = $data.isExpanded ? item2.productModel.description : $options.truncatedDescription) != null ? _a2 : ""}</span>`);
        ...
        _push(`</p><p class="font-semibold"> Price: Ksh ${ssrInterpolate($options.formattedPrice(item2.productModel.price))}</p><p class="font-semibold">Quantity: ${ssrInterpolate(item2.quantity)}</p></div></div></div>`);
      });
      _push(`<!--]--></div></div></div>`);
    }
    _push(`</div>`);
...
//# sourceMappingURL=Orders-CXRkVn2V.mjs.map
