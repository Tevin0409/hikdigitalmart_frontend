...
import { _ as __nuxt_component_2, s as script$H } from './index-BJXpECdB.mjs';
import { useSSRContext, resolveComponent, mergeProps, withCtx, createVNode, toDisplayString, openBlock, createElementBlock, createElementVNode, normalizeClass, normalizeStyle, renderSlot, toHandlers, createBlock, resolveDynamicComponent, createCommentVNode, resolveDirective, withDirectives, createSlots, Fragment, renderList, createTextVNode, normalizeProps, withModifiers, Transition, withKeys, ref, computed } from 'vue';
import { focus, getFirstFocusableElement, getLastFocusableElement, isFocusableElement, createElement, getSelection, clearSelection, setAttribute, invokeElementMethod, getAttribute, getNextElementSibling, getOuterWidth, getPreviousElementSibling, getOuterHeight, addStyle, absolutePosition, isTouchDevice, getIndex, isClickable, findSingle, find, exportCSV, getOffset, isRTL, getHiddenElementOuterWidth, getHiddenElementOuterHeight, removeClass, addClass } from '@primeuix/utils/dom';
import { isNotEmpty, resolveFieldData, equals, localeComparator, sort, findIndexInList, reorderArray, isEmpty } from '@primeuix/utils/object';
import { B as BaseStyle, _ as _export_sfc, h as FilterOperator, F as FilterService, j as FilterMatchMode, b as useProductStore, a as useNuxtApp } from './server.mjs';
import { s as script$2$3, O as OverlayEventBus, g as getVNodeProp, U as UniqueComponentId, _ as _default, C as ConnectedOverlayScrollHandler, a as script$G } from './index-BZ-8HqfV.mjs';
import { s as script$x } from './index-BBqC3hNY.mjs';
import { s as script$B, a as script$2$5, b as script$3$2 } from './index-CHf7NZjU.mjs';
import { s as script$y } from './index-CBz_SNTb.mjs';
import { B as BaseDirective, R as Ripple } from './index-Dvqyfh34.mjs';
import { s as script$2$4, a as script$A } from './index-Dq5VkPko.mjs';
import { b as script$C, s as script$1$6 } from './index-XLr1qd4A.mjs';
import { s as script$D } from './index-BgIJ0E90.mjs';
import { s as script$1$5, a as script$E } from './index-DSnb32Yv.mjs';
import { s as script$z } from './index-BFNtOUMz.mjs';
import { ZIndex } from '@primeuix/utils/zindex';
import { s as script$F } from './index-zZrFrjQS.mjs';
import { ssrRenderAttrs, ssrInterpolate, ssrRenderStyle, ssrRenderComponent, ssrRenderAttr, ssrRenderList, ssrRenderClass } from 'vue/server-renderer';
...
var classes$3 = {
  paginator: function paginator(_ref2) {
    ...
    return ["p-paginator p-component", _defineProperty$d({
...
var script$r = {
  ...
  components: {
    ...
    AngleUpIcon: script$s,
    AngleDownIcon: script$t
  }
};
...
var script$d = {
  ...
  components: {
    ...
    ArrowDownIcon: script$w,
    ArrowUpIcon: script$v,
...
const _sfc_main$3 = {
  ...
  setup() {
    ...
    const getCategories = async () => {
      ...
      try {
        await productStore2.getCategories();
        categories.value = productStore2.categories;
      } catch (error) {
...
function _sfc_ssrRender$2(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_Carousel = script$H;
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "container mx-auto p-4" }, _attrs))}><div class="flex gap-3"><div class="flex flex-col w-1/4 border rounded-lg dark:border-orange-600"><h4 class="font-bold px-4 py-4 border-b">Available Categories</h4><!--[-->`);
  ssrRenderList($setup.categories, (category, index) => {
    _push(`<div class="p-4 rounded-lg hover:bg-[#f44336] hover:text-white transition duration-300 cursor-pointer"><h5 class="font-semibold">${ssrInterpolate(category.name)}</h5></div>`);
  });
  _push(`<!--]--></div><div class="border rounded-lg border-primary w-3/4 p-3"><h2 class="text-xl font-bold mb-4">Trending Products</h2>`);
  _push(ssrRenderComponent(_component_Carousel, {
    d_circular: true,
    circular: "",
    autoplayInterval: 3e3,
    value: $setup.chunkedProducts,
    numVisible: 1,
    numScroll: 1,
    ...
    item: withCtx((slotProps, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<div class="grid grid-cols-4 gap"${_scopeId}><!--[-->`);
        ssrRenderList(slotProps.data, (product, index) => {
          _push2(`<div class="p-4 border-gray-500 rounded-lg bg-white shadow"${_scopeId}><div class="relative"${_scopeId}><img${ssrRenderAttr("src", product.image)}${ssrRenderAttr("alt", product.name)} class="object-cover rounded mb-2" style="${ssrRenderStyle({ "max-width": "100%", "height": "150px" })}"${_scopeId}></div><div class="font-medium text-center mb-2"${_scopeId}>${ssrInterpolate(product.name)}</div><div class="font-semibold text-xl text-center text-primary"${_scopeId}>${ssrInterpolate(product.price)}</div><div class="stock text-sm text-gray-600 mt-4"${_scopeId}><div class="relative"${_scopeId}><div class="h-2 bg-primary rounded"${_scopeId}><div class="h-2 bg-primary rounded" style="${ssrRenderStyle({ "width": "65%" })}"${_scopeId}></div></div><div class="flex justify-between text-xs mt-2"${_scopeId}><span${_scopeId}>Available: 334</span><span${_scopeId}>Stock: 180</span></div></div></div></div>`);
        });
        _push2(`<!--]--></div>`);
      } else {
        return [
          createVNode("div", { class: "grid grid-cols-4 gap" }, [
            (openBlock(true), createBlock(Fragment, null, renderList(slotProps.data, (product, index) => {
              return openBlock(), createBlock("div", {
                ...
                createVNode("div", { class: "font-medium text-center mb-2" }, toDisplayString(product.name), 1),
                createVNode("div", { class: "font-semibold text-xl text-center text-primary" }, toDisplayString(product.price), 1),
                createVNode("div", { class: "stock text-sm text-gray-600 mt-4" }, [
                  createVNode("div", { class: "relative" }, [
                    createVNode("div", { class: "h-2 bg-primary rounded" }, [
                      createVNode("div", {
                        class: "h-2 bg-primary rounded",
                        style: { "width": "65%" }
                      })
                    ]),
                    createVNode("div", { class: "flex justify-between text-xs mt-2" }, [
                      createVNode("span", null, "Available: 334"),
                      createVNode("span", null, "Stock: 180")
                    ])
                  ])
                ])
              ]);
...
_sfc_main$3.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/cards/home.vue");
...
const TopDeal = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["ssrRender", _sfc_ssrRender$1]]);
const _sfc_main$1 = {
  __name: "NewArrivals",
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "mx-auto" }, _attrs))} data-v-840da787><div class="flex items-center justify-between" data-v-840da787><h2 class="text-lg font-bold" data-v-840da787> Hot <span class="text-red-500" data-v-840da787>New Arrival</span> You May Like </h2><div class="flex space-x-4 border-b" data-v-840da787><!--[-->`);
      ssrRenderList(tabs.value, (tab) => {
        _push(`<button class="${ssrRenderClass([
          "px-4 py-2 text-sm font-semibold",
          selectedTab.value === tab ? "border-b-2 border-orange-500 text-orange-500" : "text-gray-600"
        ])}" data-v-840da787>${ssrInterpolate(tab)}</button>`);
      });
      _push(`<!--]--></div></div>`);
      _push(ssrRenderComponent(_component_Carousel, {
        value: productsWithRecent.value,
        numVisible: 5,
        numScroll: 1,
        class: "mt-4",
        showIndicators: false,
        ...
        item: withCtx((slotProps, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="border-r border-gray-200 rounded-lg p-4 text-center bg-white shadow-md relative group" data-v-840da787${_scopeId}>`);
            ...
            _push2(`<img${ssrRenderAttr("src", slotProps.data.image)} class="h-32 mx-auto mb-2 transition-transform group-hover:scale-105" alt="Product Image" data-v-840da787${_scopeId}><div class="flex flex-col items-center text-center p-2" data-v-840da787${_scopeId}><h3 class="text-md font-medium truncate" data-v-840da787${_scopeId}>${ssrInterpolate(slotProps.data.product.name)} <br data-v-840da787${_scopeId}><span class="text-sm font-regular" data-v-840da787${_scopeId}>${ssrInterpolate(slotProps.data.name)}</span></h3><div class="flex justify-center mt-1" data-v-840da787${_scopeId}><!--[-->`);
            ssrRenderList(5, (star) => {
              _push2(`<span class="text-yellow-400" data-v-840da787${_scopeId}><i class="${ssrRenderClass({
                "pi pi-star-fill": star <= slotProps.data.rating,
                "pi pi-star": star > slotProps.data.rating
              })}" data-v-840da787${_scopeId}></i></span>`);
            });
            _push2(`<!--]--></div><p class="text-red-500 font-semibold" data-v-840da787${_scopeId}>${ssrInterpolate(formattedPrice(slotProps.data.price))} <span class="line-through text-gray-400" data-v-840da787${_scopeId}>${ssrInterpolate(formattedPrice(slotProps.data.oldPrice))}</span></p></div><div class="absolute top-4 right-4 flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity" data-v-840da787${_scopeId}><button class="bg-white p-2 rounded-full shadow hover:bg-gray-100" data-v-840da787${_scopeId}><i class="pi pi-shopping-cart text-gray-600" data-v-840da787${_scopeId}></i></button><button class="bg-white p-2 rounded-full shadow hover:bg-gray-100" data-v-840da787${_scopeId}><i class="pi pi-heart text-gray-600" data-v-840da787${_scopeId}></i></button></div></div>`);
          } else {
            return [
              createVNode("div", { class: "border-r border-gray-200 rounded-lg p-4 text-center bg-white shadow-md relative group" }, [
                ...
                createVNode("div", { class: "flex flex-col items-center text-center p-2" }, [
                  createVNode("h3", { class: "text-md font-medium truncate" }, [
                    createTextVNode(toDisplayString(slotProps.data.product.name) + " ", 1),
                    createVNode("br"),
                    createVNode("span", { class: "text-sm font-regular" }, toDisplayString(slotProps.data.name), 1)
                  ]),
                  createVNode("div", { class: "flex justify-center mt-1" }, [
                  ...
                  createVNode("p", { class: "text-red-500 font-semibold" }, [
                    createTextVNode(toDisplayString(formattedPrice(slotProps.data.price)) + " ", 1),
                    createVNode("span", { class: "line-through text-gray-400" }, toDisplayString(formattedPrice(slotProps.data.oldPrice)), 1)
                  ])
                ]),
                createVNode("div", { class: "absolute top-4 right-4 flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity" }, [
                  createVNode("button", { class: "bg-white p-2 rounded-full shadow hover:bg-gray-100" }, [
                    createVNode("i", { class: "pi pi-shopping-cart text-gray-600" })
                  ]),
                  createVNode("button", { class: "bg-white p-2 rounded-full shadow hover:bg-gray-100" }, [
                    createVNode("i", { class: "pi pi-heart text-gray-600" })
                  ])
                ])
              ])
            ];
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/cards/NewArrivals.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const NewArrivals = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-840da787"]]);
const _sfc_main = {
  name: "ProductSection",
  components: { CardsHome: __nuxt_component_1, TopDeal, NewArrivals, Footer: __nuxt_component_2 },
  data() {
    return {
      price: "",
      images: [
      ...
      selectedImage: "https://html.themexriver.com/radios/assets/img/product/img_53.png",
      // Default image
      products: [
        {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  const _component_Carousel = script$H;
  const _component_Tag = script;
  const _component_NewArrivals = resolveComponent("NewArrivals");
  const _component_CardsHome = __nuxt_component_1;
  const _component_TopDeal = resolveComponent("TopDeal");
  const _component_Footer = __nuxt_component_2;
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "product-section pt-32" }, _attrs))} data-v-080f3396><div class="flex justify-center items-start" data-v-080f3396><div class="product-details w-1/3" data-v-080f3396><div class="badge text-primary text-sm font-semibold mb-2" data-v-080f3396> 100% Best Product </div><h1 class="text-4xl font-bold mb-4" data-v-080f3396>HD Bullet Camera</h1><p class="text-gray-500 text-lg mb-6" data-v-080f3396>Wide Angle</p><div class="price flex items-center text-2xl font-semibold text-red-600 mb-4" data-v-080f3396>${ssrInterpolate($data.price)} <span class="line-through text-gray-400 text-lg ml-4" data-v-080f3396>KES 2,860</span></div><div class="discount flex items-center mb-6" data-v-080f3396><div class="flex items-center text-lg text-primary mr-4" data-v-080f3396><span class="text-xl font-bold" data-v-080f3396>29%</span> off </div></div><div class="stock flex items-center text-sm text-gray-600 mb-6" data-v-080f3396><div class="w-full max-w-xs" data-v-080f3396><div class="relative" data-v-080f3396><div class="h-2 bg-gray-300 rounded" data-v-080f3396><div class="h-2 bg-primary rounded" style="${ssrRenderStyle({ "width": "65%" })}" data-v-080f3396></div></div></div><div class="flex justify-between text-xs mt-2" data-v-080f3396><span data-v-080f3396>Available: 334</span><span data-v-080f3396>Stock: 180</span></div></div></div>`);
  _push(ssrRenderComponent(_component_NuxtLink, { to: "/dashboard" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<button class="px-6 py-3 bg-primary text-white rounded-lg shadow-lg hover:bg-seconary" data-v-080f3396${_scopeId}> SHOP NOW \u2192 </button>`);
      } else {
        return [
          createVNode("button", { class: "px-6 py-3 bg-primary text-white rounded-lg shadow-lg hover:bg-seconary" }, " SHOP NOW \u2192 ")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div><div class="dynamic-image mt- flex justify-center items-center relative w-1/3" data-v-080f3396><img${ssrRenderAttr("src", $data.selectedImage)} alt="Selected Product" class="w-full h-full rounded- shadow- border-0" data-v-080f3396><div class="absolute bottom-0 left-0 text-white bg-primary text-sm px-3 py-1 rounded-full" style="${ssrRenderStyle({ "transform": "translate(-50%, 50%)" })}" data-v-080f3396> 29% off </div></div><div class="w-1/3 pa-6 ma-5 pl-40 h-40 dark:border-orange-600" data-v-080f3396><div class="border border-red-600 dark:border-orange-600 rounded m-2 p-4" data-v-080f3396><div class="mb-4 border-b pb-2 font-medium text-dark-600 text-center" data-v-080f3396> Top Product </div>`);
  _push(ssrRenderComponent(_component_Carousel, {
    d_circular: true,
    autoplayInterval: 3e3,
    value: $data.products,
    ...
    item: withCtx((slotProps, _push2, _parent2, _scopeId) => {
      ...
      if (_push2) {
        _push2(`<div data-v-080f3396${_scopeId}><div class="mb-4" data-v-080f3396${_scopeId}><div class="relative mx-auto" data-v-080f3396${_scopeId}><img${ssrRenderAttr("src", (_a = slotProps.data) == null ? void 0 : _a.image)}${ssrRenderAttr("alt", (_b = slotProps.data) == null ? void 0 : _b.name)} class="object-cover rounded" style="${ssrRenderStyle({ "max-width": "100%" })}" data-v-080f3396${_scopeId}>`);
        _push2(ssrRenderComponent(_component_Tag, {
          value: slotProps.data.inventoryStatus,
          severity: $options.getSeverity(slotProps.data.inventoryStatus),
          class: "absolute",
          style: { "left": "5px", "top": "5px" }
        }, null, _parent2, _scopeId));
        _push2(`</div></div><div class="mb-4 font-medium flex justify-around" data-v-080f3396${_scopeId}>${ssrInterpolate((_c = slotProps.data) == null ? void 0 : _c.name)}</div><div class="flex justify-between items-center flex justify-around" data-v-080f3396${_scopeId}><div class="mt-0 font-semibold text-xl flex justify-around py-4" data-v-080f3396${_scopeId}>${ssrInterpolate(slotProps.data.price)}</div></div><div class="stock flex items-center text-sm text-gray-600 mb-6 flex justify-around" data-v-080f3396${_scopeId}><div class="w-full max-w-xs" data-v-080f3396${_scopeId}><div class="relative" data-v-080f3396${_scopeId}><div class="h-2 bg-primary rounded" data-v-080f3396${_scopeId}><div class="h-2 bg-primary rounded" style="${ssrRenderStyle({ "width": "65%" })}" data-v-080f3396${_scopeId}></div></div><div class="flex justify-between text-xs mt-2" data-v-080f3396${_scopeId}><span data-v-080f3396${_scopeId}>Available: 334</span><span data-v-080f3396${_scopeId}>Stock: 180</span></div></div></div></div></div>`);
      } else {
        return [
          createVNode("div", null, [
            createVNode("div", { class: "mb-4" }, [
            ...
            createVNode("div", { class: "mb-4 font-medium flex justify-around" }, toDisplayString((_f = slotProps.data) == null ? void 0 : _f.name), 1),
            createVNode("div", { class: "flex justify-between items-center flex justify-around" }, [
              createVNode("div", { class: "mt-0 font-semibold text-xl flex justify-around py-4" }, toDisplayString(slotProps.data.price), 1)
            ]),
            createVNode("div", { class: "stock flex items-center text-sm text-gray-600 mb-6 flex justify-around" }, [
  ...
  _push(`</div></div></div><div class="carousel flex justify-center items-center mt-8 space-x-4" data-v-080f3396><button class="text-gray-600 hover:text-gray-800 px-3 py-2 bg-gray-100 rounded-full shadow-md" data-v-080f3396> \u2190 </button><div class="flex items-center space-x-4" data-v-080f3396><!--[-->`);
  ssrRenderList($data.images, (image, index) => {
    _push(`<img${ssrRenderAttr("src", image)}${ssrRenderAttr("alt", "Product " + (index + 1))} class="${ssrRenderClass([{ "border-orange-500": $data.selectedImage === image }, "w-16 h-16 rounded-md border border-gray-300 cursor-pointer hover:border-orange-500"])}" data-v-080f3396>`);
  });
  _push(`<!--]--></div><button class="text-gray-600 hover:text-gray-800 px-3 py-2 bg-gray-100 rounded-full shadow-md" data-v-080f3396> \u2192 </button></div><div class="feature__wrap grid grid-cols-5 gap-2 m-12" data-v-080f3396><!--[-->`);
  ssrRenderList($data.features, (feature, index) => {
    _push(`<div class="feature__item flex flex-col items-center text-center" data-v-080f3396><div class="icon" data-v-080f3396><i class="${ssrRenderClass(feature.icon)}" style="${ssrRenderStyle({ "font-size": "2rem", "color": "#f44336" })}" data-v-080f3396></i></div><div class="content mt-4" data-v-080f3396><h3 class="text-lg font-semibold" data-v-080f3396>${ssrInterpolate(feature.title)}</h3><p class="text-sm text-gray-600" data-v-080f3396>${ssrInterpolate(feature.description)}</p></div></div>`);
  });
  _push(`<!--]--></div>`);
  _push(ssrRenderComponent(_component_NewArrivals, { class: "py-12" }, null, _parent));
  _push(ssrRenderComponent(_component_CardsHome, null, null, _parent));
  _push(ssrRenderComponent(_component_TopDeal, null, null, _parent));
  _push(ssrRenderComponent(_component_Footer, { class: "w-full left-0 bottom-0 bg-gray-100 text-gray-800 py-10" }, null, _parent));
  _push(`</div>`);
}
...
const Dashboard = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender], ["__scopeId", "data-v-080f3396"]]);

export { Dashboard as default };
//# sourceMappingURL=Home-D4A8rmA_.mjs.map
