...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const items = ref([
      { label: "Quotations", route: "/cart" },
      { label: "Quotation", route: "/checkout" }
    ]);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (cartItems.value.length < 1) {
        ...
        _push(`<div class="mx-auto container"><div class="bg-red-10 rounded-lg p-4"><div class="flex justify-between items-center mb-4"><h3 class="text-lg font-semibold text-gray-700"> Quotation (${ssrInterpolate(cartItems.value.length)}) </h3>`);
        _push(ssrRenderComponent(_component_NuxtLink, { to: "/quotation" }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
        ...
        ssrRenderList(cartItems.value, (item) => {
          ...
          _push(`<div class="rounded-lg p-4 gap-3 pb-4 mb-2 border cursor-pointer bg-white shadow-sm"><div class="flex flex-wrap items-start gap-4"><img class="w-16 h-16 sm:w-20 sm:h-20 object-contain"${ssrRenderAttr(
            "src",
            (_a2 = (_c = (_b = (_a = item.productModel) == null ? void 0 : _a.images) == null ? void 0 : _b.find(
              (image) => image.isPrimary
            )) == null ? void 0 : _c.uploadUrl) != null ? _a2 : (_e = (_d = item.images) == null ? void 0 : _d.find((image) => image.isPrimary)) == null ? void 0 : _e.uploadUrl
          ...
          _push(`<!--]--></p></div><div class="text-right"><p class="text-lg sm:text-xl font-semibold text-gray-900"> Ksh ${ssrInterpolate(formattedPrice((_c2 = (_h = item.productModel) == null ? void 0 : _h.price) != null ? _c2 : item.price))}</p><div class="flex items-center"><button class="bg-gray-300 px-2 sm:px-3 py-1 rounded-lg disabled:opacity-50"${ssrIncludeBooleanAttr(item.quantity <= 1) ? " disabled" : ""}> - </button><span class="px-3 sm:px-4 text-sm sm:text-base">${ssrInterpolate(item.quantity)}</span><button class="bg-primary text-white px-2 sm:px-3 py-1 rounded-lg"> + </button></div></div><div class="flex flex-wrap items-center justify-between mt-4"><button class="text-red-500 flex items-center text-sm sm:text-base"><span class="mr-2"> Remove \u{1F5D1}\uFE0F</span></button></div></div></div>`);
        });
        _push(`<!--]--></div><div class="col-span-12 md:col-span-3"><div class="cart-total rounded-lg p4 bg-white shadow"><h3 class="text-lg font-semibold mb-2">Estimated Total</h3><p class="text-gray-600"> Subtotal: Ksh ${ssrInterpolate(formattedPrice(cartTotal.value))}</p><div class="flex items-center justify-between my-4"><label class="text-gray-600">Apply VAT (16%)</label><label class="relative inline-flex items-center cursor-pointer"><input type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(isVat.value) ? ssrLooseContain(isVat.value, null) : isVat.value) ? " checked" : ""} class="sr-only peer"><div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-primary transition-colors"></div><div class="absolute left-1 top-1 bg-white border border-gray-300 w-4 h-4 rounded-full transition-transform peer-checked:translate-x-5"></div></label></div>`);
        if (isVat.value) {
          _push(`<hr class="my-2">`);
        } else {
          _push(`<!---->`);
        ...
        _push(`<hr class="my-2"><p class="text-lg font-bold"> Total Estimate: Ksh ${ssrInterpolate(formattedPrice(cartTotal.value + (isVat.value ? getVat(cartTotal.value) : 0)))}</p><button class="w-full bg-primary text-white py-2 mt-4 rounded-lg hover:bg-secondary transition"> Request Quotation (${ssrInterpolate(formattedPrice(cartTotal.value + (isVat.value ? getVat(cartTotal.value) : 0)))}) </button></div></div></div></div></div></div>`);
      }
      _push(`</div>`);
    };
  }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/quotation/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=index-BzlnyzU5.mjs.map
