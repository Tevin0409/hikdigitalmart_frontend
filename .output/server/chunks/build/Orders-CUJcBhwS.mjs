...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  if (!$data.detailedView && !$data.fetching) {
    ...
    if ($data.fetching) {
      ...
      _push(`<div><a class="p-button-text text-primary hover:text-white cursor-pointer" style="${ssrRenderStyle({ "color": "var(--secondary-color) !important", "text-decoration": "underline" })}"> Back to Orders </a><div class="p-6 bg-white rounded-lg"><div class="flex justify-between"><h2 class="text-2xl font-semibold">Order Details</h2><div class="flex items-center gap-2"><p class="font-semibold">Status</p><span class="${ssrRenderClass([{
      ...
      }, "px-3 py-1 text-xs rounded-full"])}">${ssrInterpolate($data.orderDetails.status)}</span></div></div>`);
      _push(ssrRenderComponent(_component_Divider, null, null, _parent));
      _push(`<div class="grid grid-cols-1 md:grid-cols-4 gap-4"><div><p class="font-semibold">Order ID</p><p>${ssrInterpolate($data.orderDetails.id)}</p></div><div><p class="font-semibold">Customer Name</p><p>${ssrInterpolate($data.orderDetails.first_name)} ${ssrInterpolate($data.orderDetails.last_name)}</p></div><div><p class="font-semibold">Company Name</p><p>${ssrInterpolate($data.orderDetails.company_name || "N/A")}</p></div><div><p class="font-semibold">Address</p><p>${ssrInterpolate($data.orderDetails.street_address)}, ${ssrInterpolate($data.orderDetails.town)}</p></div><div><p class="font-semibold">Phone Number</p><p>${ssrInterpolate($data.orderDetails.phone_number)}</p></div><div><p class="font-semibold">Email</p><p>${ssrInterpolate($data.orderDetails.email)}</p></div><div><p class="font-semibold">Order Price</p><p>Ksh ${ssrInterpolate($options.formattedPrice($data.orderDetails.orderPrice))}</p></div><div><p class="font-semibold">VAT</p><p>Ksh ${ssrInterpolate($options.formattedPrice($data.orderDetails.vat))}</p></div><div><p class="font-semibold">Total</p><p>Ksh ${ssrInterpolate($options.formattedPrice($data.orderDetails.total))}</p></div><div><p class="font-semibold">Order Date</p><p>${ssrInterpolate($options.formatDate($data.orderDetails.createdAt))}</p></div></div>`);
      ...
      ssrRenderList($data.orderDetails.orderItems, (item2, index) => {
        var _a, _b;
        _push(`<div class="border rounded-lg p-4"><div class="flex items-center space-x-4"><img${ssrRenderAttr(
          "src",
          ((_b = (_a = item2.productModel.images) == null ? void 0 : _a[0]) == null ? void 0 : _b.optimizeUrl) || "https://via.placeholder.com/100"
...
//# sourceMappingURL=Orders-CUJcBhwS.mjs.map
