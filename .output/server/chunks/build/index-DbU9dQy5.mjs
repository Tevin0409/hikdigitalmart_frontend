...
import { ssrRenderAttrs, ssrRenderAttr, ssrRenderComponent, ssrRenderClass, ssrInterpolate, ssrRenderList, ssrIncludeBooleanAttr, ssrLooseContain } from 'vue/server-renderer';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (cartItems.value.length < 1) {
        ...
        ssrRenderList(cartItems.value, (item) => {
          ...
          _push(`<div class="rounded-lg p-4 gap-3 pb-4 mb-2 border cursor-pointer"><div class="flex items-start"><img class="w-20 h-20 object-contain"${ssrRenderAttr(
            "src",
            (_a2 = (_b = (_a = item.productModel) == null ? void 0 : _a.images.find((image) => image.isPrimary)) == null ? void 0 : _b.uploadUrl) != null ? _a2 : (_d = (_c = item.images) == null ? void 0 : _c.find((image) => image.isPrimary)) == null ? void 0 : _d.uploadUrl
            // item.images[0]
          )} alt="Product Image"><div class="ml-4 flex-grow"><h2 class="text-lg font-medium">${ssrInterpolate((_b2 = (_e = item.productModel) == null ? void 0 : _e.name) != null ? _b2 : item.name)}</h2><p class="text-gray-600 truncate"><!--[-->`);
          ssrRenderList(item.features || item.productModel.features, (item2) => {
            _push(`<span class="font-bold">${ssrInterpolate(item2.description)}</span>`);
          });
          _push(`<!--]--></p></div><div class="text-right"><p class="text-xl font-semibold text-gray-900"> Ksh ${ssrInterpolate(formattedPrice((_c2 = (_f = item.productModel) == null ? void 0 : _f.price) != null ? _c2 : item.price))}</p></div></div><div class="flex items-center justify-between mt-4"><button class="text-red-500 flex items-center"><span class="mr-2">\u{1F5D1}\uFE0F</span> Remove </button><div class="flex items-center"><button class="bg-gray-300 px-3 py-1 rounded-lg"${ssrIncludeBooleanAttr(item.quantity <= 1) ? " disabled" : ""}> - </button><span class="px-4">${ssrInterpolate(item.quantity)}</span><button class="bg-primary text-white px-3 py-1 rounded-lg"> + </button></div></div></div>`);
        });
        _push(`<!--]--></div><div class="col-span-12 md:col-span-3"><div class="cart-total rounded-lg p-4 bg-white shadow"><h3 class="text-lg font-semibold mb-2">Cart Total</h3><p class="text-gray-600"> Subtotal: Ksh ${ssrInterpolate(formattedPrice(cartTotal.value))}</p><div class="flex items-center justify-between my-4"><label class="text-gray-600">Apply VAT (16%)</label><label class="relative inline-flex items-center cursor-pointer"><input type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(applyVat.value) ? ssrLooseContain(applyVat.value, null) : applyVat.value) ? " checked" : ""} class="sr-only peer"><div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-primary transition-colors"></div><div class="absolute left-1 top-1 bg-white border border-gray-300 w-4 h-4 rounded-full transition-transform peer-checked:translate-x-5"></div></label></div><hr class="my-2">`);
        if (applyVat.value) {
          _push(`<p class="text-gray-600"> VAT (16%): Ksh ${ssrInterpolate(formattedPrice(getVat(cartTotal.value)))}</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<hr class="my-2"><p class="text-lg font-bold"> Total: Ksh ${ssrInterpolate(formattedPrice(
          cartTotal.value + (applyVat.value ? getVat(cartTotal.value) : 0)
        ))}</p><button class="w-full bg-primary text-white py-2 mt-4 rounded-lg hover:bg-secondary transition"> Checkout (${ssrInterpolate(formattedPrice(
          cartTotal.value + (applyVat.value ? getVat(cartTotal.value) : 0)
...
//# sourceMappingURL=index-DbU9dQy5.mjs.map
