...
import { s as script$5, a as script$2$2, b as script$3$2 } from './index-CEOEMxBM.mjs';
...
import { a as script$6, U as UniqueComponentId, C as ConnectedOverlayScrollHandler, O as OverlayEventBus, s as script$7$1, b as script$2$3 } from './index-NgWSRoHh.mjs';
...
import { s as script$3$1, b as script$7 } from './index-CPSBx0zQ.mjs';
...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div>`);
      if (_ctx.isWholesaler) {
        _push(`<button class="relative text-3xl text-gray-700 hover:text-blue-600"><i class="pi pi-clipboard"></i>`);
        if (_ctx.quotationList.length) {
          _push(`<span class="absolute top-0 right-0 bg-red-500 text-white text-xs px-1.5 rounded-full">${ssrInterpolate(_ctx.quotationList.length)}</span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</button>`);
      ...
      _push(`</div><nav class="hidden md:flex lg:flex w-1/5 justify-between items-center p-4">`);
      ...
      _push(`<div class="ml-4 hidden sm:flex flex-col font-bold"><span class="text-xs text-gray-400">My Cart</span><span>Ksh ${ssrInterpolate(formattedPrice(cartTotal.value))}</span></div>`);
      if (_ctx.isWholesaler) {
        _push(`<button class="relative text-3xl text-gray-700 hover:text-blue-600"><i class="pi pi-clipboard"></i>`);
        if (_ctx.quotationList.length) {
          _push(`<span class="absolute top-0 right-0 bg-red-500 text-white text-xs px-1.5 rounded-full">${ssrInterpolate(_ctx.quotationList.length)}</span>`);
...
//# sourceMappingURL=Navbar-DDndF78N.mjs.map
